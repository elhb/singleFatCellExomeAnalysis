...
my $totalBases; my $trimmedBases;my $trimmedBasesOLD; $totalBases = 0; $trimmedBases = 0;
...
#	$trimmedBasesOLD = $trimmedBases;
	if ($pos==0) {$trimmedBases += length($q); $s = "N\n"; $q = "#\n"}  # scanned whole read and didn't integrate to zero?  replace with "empty" read ...
	else {  # integrated to zero?  trim before position where area reached a maximum (~where string of qualities were still below 20 ...)
		$trimmedBases += length($q)-$maxPos;
		$s = substr($s,0,$maxPos)."\n";
		$q = substr($q,0,$maxPos)."\n";
	}
#	if ($trimmedBasesOLD != $trimmedBases) {
#		print STDERR $h1.$s.$h2.$q;
#	}
	
	print $h1.$s.$h2.$q;
}
print STDERR $totalBases;
print STDERR "\tbases\n";
print STDERR $trimmedBases;
